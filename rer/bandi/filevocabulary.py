...
class XMLFileVocabulary(object):
    ...
    def __init__(self, envvar, vocabulary_name, default_terms=None):
        ...
        if envvar not in os.environ:
            raise ConfigurationError(
                "Missing environment variable %s" % envvar
            )
        ...
        if not os.path.exists(filename):
            ...
            fout.write(
                self.default_file_contents(vocabulary_name, default_terms)
            )
            fout.close()
            raise ConfigurationError(
                "Vocabulary file file %s created with default values. Please check the contents and run again."
                % filename
            )
        ...
        try:
            ...
            raise ConfigurationError(
                "Invalid XML in file %s: %s" % (fin.name, e)
            )
    ...
    def extract_terms(self, xml, vocabulary_name):
        terms = []
        for el_term in xml.getroot().xpath(
            "//vocabulary[@name='%s']/term" % vocabulary_name
        ):
